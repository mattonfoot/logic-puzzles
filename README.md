...
**[Every screen, as the build draws it](docs/screenshots.md)** — twenty-three
captures from the real app, regenerated by `npm run screenshots` and walked
again in CI. **[On an iPad](docs/ipad.md)** is the same walk at tablet
size, which is a different and less flattering story — and the reason
`app.json` sets `ios.supportsTablet: false`, so the App Store offers Deduction
as an iPhone app and an iPad owner gets the phone build letterboxed rather than
the phone layout stranded on a tablet.
...
then pyramids of three and ten pips standing on their point — both are
triangular numbers, so both stack with none left over — and, for the hundred,
the same struck edge with a
